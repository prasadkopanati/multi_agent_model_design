...
Read `.spiq/research.md`. It contains technical documentation, API references, code
samples, and key findings gathered by the research agent specifically for this feature.
Use it when estimating task complexity, deciding which third-party libraries to use, and
writing the acceptance criteria for each task.

If `.spiq/research.md` does not exist (pipeline resumed at plan stage with no prior research):
- Read `.spiq/skills/RESEARCH.md`
- Run targeted research for any external APIs or libraries the spec references
- Save findings to `.spiq/research.md` before proceeding to Step 1
